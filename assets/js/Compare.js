				...
				while(splitValOne[counter] && splitValTwo[counter]){
					if(splitValOne[counter].split(',')[0] == splitValTwo[counter].split(',')[0]){
							...
							if(splitValOne[counter] == splitValTwo[counter]){
								...
								//document.getElementById('results').innerHTML = document.getElementById('results').innerHTML + '<p class="DifferentTag">Line ' + counter + ' is not the same : ' + splitValOne[counter] + ' // ' + splitValTwo[counter] + '</p>';
								counter++;
							}
					}else{
						counter++;
